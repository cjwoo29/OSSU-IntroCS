...
class PlaintextMessage(Message):
    def __init__(self, input_text, pad=None):
        ...
        self.pad = list(pad) if pad!=None else self.generate_pad()
    ...
    def generate_pad(self):
        ...
        '''
        return [random.randint(0,109) for i in range(len(self.get_text()))]
...
class EncryptedMessage(Message):
    ...
    def decrypt_message(self, pad):
        ...
        if len(pad) != len(txt): raise ValueError
        result = []
        neg_pad = [i*(-1) for i in pad]
        for i in range(len(neg_pad)):
            result.append(self.shift_char(txt[i], neg_pad[i]))
        return PlaintextMessage(''.join(result), pad)
